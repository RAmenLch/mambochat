export default {
  ...
  // 多模态描述
  multimodalDescriber: '多模态描述',
  multimodalDescriberEnable: '启用多模态描述',
  multimodalDescriberEnableDesc: '为主模型补充多模态理解能力。主模型原生支持的模态会直接透传；已绑定模型的模态由描述模型生成描述文本；未绑定的模态将返回显式拒绝提示，避免多模态文件直通模型 API 报错。',
  multimodalDescriberImageLabel: '图片描述模型',
  multimodalDescriberAudioLabel: '音频描述模型',
  multimodalDescriberVideoLabel: '视频描述模型',
  ...
  goalLoop: {
    title: '任务循环',
    enableDesc: '让 Agent 自动多轮执行，直到目标完成、条件满足或达到轮数上限',
    mode: '循环模式',
    modeLlm: '交给AI自己规划',
    modeLlmDesc: '适合长程任务。AI 自己拆解目标并持续执行，直到完成、卡住或轮数用尽',
    modePreset: '按我的规则执行',
    modePresetDesc: '每轮强制围绕设定的目标执行，完成条件全部满足后自动结束',
    maxRounds: '最大执行轮数',
    maxRoundsDesc: '到达上限后自动停止，防止无限执行',
    blockedThreshold: '卡住判定阈值',
    blockedThresholdDesc: 'AI 至少工作满设定轮数后，才允许宣告"卡住了"无法继续进行（默认 3）',
    objective: '每轮目标提示词',
    objectivePlaceholder: '例如：每一轮先调用安全检查工具，再继续执行',
    conditions: '完成条件（全部满足后自动结束）',
    conditionsDesc: '多个条件需全部满足循环才会结束；轮数用尽也会强制停止。参数匹配时，所有指定参数都相等才算一次有效调用',
    conditionToolPlaceholder: '选择工具',
    conditionTimesLabel: '至少调用',
    conditionTimesSuffix: '次',
    conditionArgs: '参数匹配',
    argKeyPlaceholder: '参数名',
    argKeyNoArgs: '该工具无参数定义，可直接输入',
    argValuePlaceholder: '参数值',
    addCondition: '添加条件',
    addArg: '添加参数',
    validateObjective: '请填写"每轮目标"',
    validateCondition: '请至少添加一条完成条件（否则只能靠轮数上限结束）',
    validateTool: '每条完成条件都需要选择工具',
    validateRounds: '最大执行轮数需 ≥ 1',
  },
  // 尾部工具调用
  tailTool: {
    title: '尾部工具调用',
    enableDesc: '每轮收尾后，用与主调用完全一致的上下文与模型，让模型通过通用工具 tail_tool 额外产出设定的尾部任务（如后续建议）。不进正文、可复用缓存',
    failMode: '未调用时处理',
    failModeSilent: '静默失败',
    failModeMessage: '产出失败事件',
    failModeDesc: '模型未调用 tail_tool 时的行为',
    failMessage: '失败提示文案',
    failMessagePlaceholder: '例如：未能生成建议',
    tasks: '尾部任务',
    tasksDesc: '每轮收尾时要求模型产出的任务。每个任务包含名称与具体使用说明',
    taskNamePlaceholder: '选择工具（任务名）',
