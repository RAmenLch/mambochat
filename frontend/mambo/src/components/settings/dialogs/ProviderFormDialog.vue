...
import type { AIProviderWithModels, ProviderWithModelsCreate, AIProviderUpdate, AIModelBase, AIModel } from '@/api/types';
...
function handleApiKeyFocus() {
  if (providerForm.apiKey === API_KEY_PLACEHOLDER) providerForm.apiKey = '';
}

function handleApiKeyBlur() {
  if (isEditing.value && providerForm.apiKey === '') providerForm.apiKey = API_KEY_PLACEHOLDER;
}
...
async function handleUpdateProvider() {
  const providerUpdateData: AIProviderUpdate = { name: providerForm.name, apiHost: providerForm.apiHost };
  if (providerForm.apiKey !== API_KEY_PLACEHOLDER) {
    providerUpdateData.apiKey = providerForm.apiKey;
  }
  const providerUpdatePromise = providerStore.updateProvider(props.providerData!.id, providerUpdateData);
  ...
  const modelsToUpdate = providerForm.models.filter(current => {
    if (!current.id) return false;
    const initial = initialModels.find(m => m.id === current.id);
    return initial && (initial.name !== current.name || initial.modelId !== current.modelId);
  }).map(m => ({ id: m.id!, data: { name: m.name, modelId: m.modelId } }));

  const addPromises = modelsToAdd.map(m => providerStore.addModel({ ...m, providerId: props.providerData!.id }));
  const deletePromises = modelsToDelete.map(m => providerStore.removeModel(m.id));
  const updatePromises = modelsToUpdate.map(m => providerStore.updateModel(m.id, m.data));
...
// 供父组件调用的方法
defineExpose({
  addFetchedModels(modelsToAdd: string[], fetchedModels: AIModelBase[]) {
