...
*   **🤖 Mambo 智能体（[Mambo Agent](https://github.com/RAmenLch/mambo_agents)）**
    ...
    *   **任务循环**：让 Agent 自动多轮执行，直到目标完成、条件满足或达到轮数上限，支持"交给 AI 自己规划"与"按我的规则执行"两种模式。
...
2. **运行安装程序**
   双击 `MamboChat-Setup-<版本号>.exe`，按向导完成安装（可选择自定义安装目录）。
