...
class ChatBasedReActInitializer(AbstractAgentInitializer):
    ...
    def __init__(
        ...
        # 前置拦截：为下一阶段独立 Agent 表接入预留空间
        if getattr(chat, "chatMode", ChatMode.NORMAL.value) == ChatMode.AGENT.value:
            raise NotImplementedError("当前版本暂不支持基于独立 Agent 表初始化，请期待后续更新。")
    ...
    async def initialize(self) -> Tuple[AgentConfig, str]:
        ...
        # 1. 资源挂载与分发 (Resource Prompt Merge)
