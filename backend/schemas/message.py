...
class SubMessageConfig(BaseModel):
    ...
    is_tail_tool: Optional[bool] = Field(None, description="是否为尾部工具调用中间件(TailToolMiddleware)产出的子消息；不参与上下文（context_participation_length=0），前端在底部可折叠小窗口中以不同颜色渲染，不进入正文")
