...
export const translations: Record<DesktopLocale, DesktopTranslations> = {
  'zh-CN': {
    ...
    'local.dataDir': '数据目录',
    'local.dataDir.placeholder': '留空使用默认位置（%AppData%）',
    'local.dataDir.browse': '选择文件夹…',
    'local.dataDir.hint': '数据库与上传文件的存储位置。可指向其他磁盘以避免占用系统盘空间。修改后需重启后端生效。',
    'dataDir.current': '当前数据目录',
    'dataDir.selectTitle': '选择数据目录',
    'dataDir.selectButton': '选择',
    'dataDir.migrateTitle': '迁移数据',
    'dataDir.migrateMessage': '检测到数据目录已更改。是否将现有数据迁移到新目录？',
    'dataDir.migrateAndDelete': '迁移并删除旧数据',
    'dataDir.migrate': '仅迁移',
    'dataDir.cancel': '取消',
    'dataDir.useTarget': '直接使用目标数据',
    'dataDir.usingTarget': '已直接使用目标目录中的现有数据',
    'dataDir.nonEmptyNote': '目标目录非空：将仅在其中创建 DB/ 与 uploads/ 子目录，不会影响已有的其他文件。',
    'dataDir.existingNote': '目标目录已包含 MamboChat 数据（DB/mambo.dat）。选择"迁移"将覆盖其中的数据。',
    'dataDir.parentNote': '目标目录是当前数据目录的父目录，不推荐使用，可能导致目录结构混乱。',
    'dataDir.migrated': '数据已迁移到新目录',
    'dataDir.migrateFailed': '数据迁移失败',
    'dataDir.noData': '新目录为空，将创建全新数据库',
    'dataDir.cancelled': '已取消，数据目录未更改',
    'dataDir.revertedDefault': '已恢复默认数据目录',
  ...
  'en': {
    ...
    'local.dataDir': 'Data Directory',
    'local.dataDir.placeholder': 'Leave empty to use the default location (%AppData%)',
    'local.dataDir.browse': 'Browse…',
    'local.dataDir.hint': 'Storage location for databases and uploads. Point it to another drive to avoid filling up the system drive. Changing this restarts the backend.',
    'dataDir.current': 'Current data directory',
    'dataDir.selectTitle': 'Select Data Directory',
    'dataDir.selectButton': 'Select',
    'dataDir.migrateTitle': 'Migrate Data',
    'dataDir.migrateMessage': 'The data directory has changed. Do you want to migrate existing data to the new location?',
    'dataDir.migrateAndDelete': 'Migrate & delete old data',
    'dataDir.migrate': 'Migrate only',
    'dataDir.cancel': 'Cancel',
    'dataDir.useTarget': 'Use existing data in target',
    'dataDir.usingTarget': 'Using the existing data in the target directory',
    'dataDir.nonEmptyNote': 'The target directory is not empty: only DB/ and uploads/ subdirectories will be created there; existing unrelated files are left untouched.',
    'dataDir.existingNote': 'The target directory already contains MamboChat data (DB/mambo.dat). Choosing "Migrate" will overwrite that data.',
    'dataDir.parentNote': 'The target directory is a parent of the current data directory. This is not recommended and may result in a messy layout.',
    'dataDir.migrated': 'Data migrated to the new directory',
    'dataDir.migrateFailed': 'Data migration failed',
    'dataDir.noData': 'The new directory is empty; a fresh database will be created',
    'dataDir.cancelled': 'Cancelled — data directory unchanged',
    'dataDir.revertedDefault': 'Reverted to the default data directory',
