...
| E5 | path 不存在 | `/nonexistent/path/xyz` | `3` | ❌ `Path '/nonexistent/path/xyz' not found.` |
...
| 排序 | 同层内按名字字母升序（与 `ls` 一致的 ASCII 序），点文件优先 |
...
| # | 用例名称 | path | 预期结果 |
|---|---------|------|----------|
| N1 | 混合目录（文件+子目录） | 项目根目录 | 列出所有条目：文件显示大小，目录以 `/` 结尾，字母序排列，隐藏文件正常显示 |
...
| F4 | 排序规则 | 按完整路径字符串字母升序（ASCII 序），点文件排在前面 |
| F5 | 零大小文件 | 显示 `(0 B)` |
| F6 | 隐藏文件 | `.` 开头的文件正常列出 |
...
| 场景 | 返回信息 |
|------|----------|
| `path` 不存在 | `Error: Path not found: <path>` |
| `path` 是根目录 `/` | `[PATH_IS_ROOT] 路径不能是根目录 '/'；请使用子目录如 '/workspace'` |
...
| # | 用例名称 | path | glob | pattern | 预期结果 |
|---|---------|------|------|---------|----------|
| E1 | path 不存在 | `/workspace/nonexistent` | `*.py` | `test` | ❌ `Error: Path not found:` |
...
| C3 | 并发编辑同一文件同一位置 | 同时 `edit` 同一文件的相同 `old_str`（如 `PART_2`） | ⚠️ 先到成功，后到因 `OLD_STR_NOT_FOUND` 失败 |
...
| 同一文件 + 相同 old_str | ⚠️ 仅一个成功 | 先完成的修改了文本，后到的找不到 old_str |
