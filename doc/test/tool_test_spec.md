...
- [第七章: `glob` — 文件与目录匹配搜索](#第七章-glob--文件与目录匹配搜索)
...
|------|------|
| **工具名** | `glob` |
| **功能** | 根据 POSIX 风格的 glob 模式，在指定路径下递归搜索匹配的文件和目录 |
| **必填参数** | `pattern` (string) — 搜索模式 |
| **可选参数** | `path` (string, 默认 `/workspace`) |

## 7.2 参数规范

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `path` | string | ❌ 否 | `/workspace` | 搜索的根目录，必须是**存在的目录**；不能是文件、根目录 `/`、不存在路径、含 `..` 的路径 |
| `pattern` | string | ✅ 是 | — | POSIX 风格 glob 模式，不能为空 |

### 支持的 glob 模式

| 模式 | 含义 | 示例 |
|------|------|------|
| `*` | 匹配零或多个任意字符（不跨路径分隔符） | `*.py` 匹配 `main.py` |
| `**` | 递归匹配任意层级的目录 | `**/*.py` 匹配任意子目录下的 `.py` 文件 |
| `?` | 匹配单个任意字符 | `main.??` 匹配 `main.py`、`main.c` |
| `[...]` | 匹配字符类中的任一字符 | `*.[jt]s` 匹配 `.js` 或 `.ts` 文件 |
| `[!...]` / `[^...]` | 匹配不在字符类中的字符 | `*.[!c]ss` 不匹配 `.css` |

> **⚠️ 不支持**: 花括号扩展（如 `*.{py,json}`）不被支持，会返回 `No matches found.`。如需要匹配多种扩展名，请使用字符类 `*.[pj]` 或分别搜索。

## 7.3 返回值规范

### 正常返回

每行一个匹配结果，格式为 `绝对路径(大小)`（文件）或 `绝对路径/`（目录）：

```
/workspace/demo_project/src/main.py(1 KB)
/workspace/demo_project/src/models/
```

- **文件**: 显示完整绝对路径 + `(大小)`，大小单位自适应（B / KB / MB / GB）
- **目录**: 显示完整绝对路径 + 尾部 `/`，无大小信息
- **排序**: 按路径字母序排列（ASCII 升序），文件与目录混合排序
- **隐藏文件**: 点号 `.` 开头的文件/目录可被正常匹配（如 `**/.?*` 匹配 `.gitignore`）
- **空目录**: 不匹配任何条目（除非路径模式本身指向该空目录）
...
|------|----------|
| `path` 不存在 | `Warning: [NOT_FOUND] 路径不存在` |
| `path` 是文件（非目录） | `Warning: [NOT_DIR] 目标是文件，不是目录` |
| `path` 是根目录 `/` | `[PATH_IS_ROOT] 路径不能是根目录 '/'；请使用子目录如 '/workspace'` |
| `path` 含 `..` | `[PATH_TRAVERSAL] path 不能包含 '..' 路径穿越` |
| `pattern` 为空 | `Warning: [INVALID] 搜索模式不能为空` |

## 7.4 测试用例

> 测试基准目录: `/workspace/demo_project`（由你自行创建，见第零章）
>
> **说明**: 以下测试用例基于第零章搭建的测试环境。文件大小等数值取决于实际创建的文件。

### 7.4.1 正常场景 — 基础匹配

| # | 用例名称 | path | pattern | 预期结果 |
|---|---------|------|---------|----------|
| N1 | 递归匹配 Python 文件 | 项目根目录 | `**/*.py` | 匹配所有子目录下的 `.py` 文件（如 `src/main.py`、`tests/test_user.py`） |
| N2 | 单层匹配 Python 文件 | 项目根目录 | `*/*.py` | 仅匹配一层目录下的 `.py` 文件（不递归，如 `src/main.py`、`tests/test_user.py`，不含更深层 `src/models/user.py`） |
| N3 | 匹配 JSON 文件 | 项目根目录 | `**/*.json` | 匹配所有 `.json` 文件 |
| N4 | 匹配 YAML 文件 | 项目根目录 | `**/*.yaml` | 匹配所有 `.yaml` 文件 |
| N5 | 匹配 Markdown 文件 | 项目根目录 | `**/*.md` | 匹配所有 `.md` 文件（含根目录和子目录） |
| N6 | 匹配 CSS 文件 | 项目根目录 | `**/*.css` | 匹配静态资源目录下的 CSS 文件 |
| N7 | 匹配 JS 文件 | 项目根目录 | `**/*.js` | 匹配 JS 文件 |
| N8 | 匹配 HTML 文件 | 项目根目录 | `**/*.html` | 匹配 HTML 模板文件 |
| N9 | 匹配 CSV 文件 | 项目根目录 | `**/*.csv` | 匹配数据文件 |
| N10 | 匹配特定子目录下的文件 | 项目根目录 | `**/config/*` | 匹配 `config/` 目录下的所有条目 |
| N11 | 匹配深层目录下的文件 | 项目根目录 | `**/static/**/*.*` | 匹配 `static/` 下任意子目录中的文件 |
| N12 | 精确文件名匹配 | 项目根目录 | `**/README*` | 匹配所有 `README` 开头的文件（含根目录和 `docs/` 下） |
| N13 | 匹配隐藏文件 | 项目根目录 | `**/.?*` | 匹配所有点号开头的文件（如 `.gitignore`、`.env.example`、`.gitkeep`） |
| N14 | 路径带尾 `/` | 项目根目录 + 末尾 `/` | `**/*.py` | 与不带尾 `/` 结果一致 |

### 7.4.2 正常场景 — glob 通配符

| # | 用例名称 | path | pattern | 预期结果 |
|---|---------|------|---------|----------|
| N15 | `?` 单字符匹配 | 项目根目录 | `**/main.??` | 匹配 `main.py`（扩展名 2 字符） |
| N16 | `?` 匹配 HTML 扩展名 | 项目根目录 | `**/*.htm?` | 匹配 `.html`（`?` 匹配 `l`） |
| N17 | 字符类 `[a-z]` | 项目根目录 | `**/[a-z]*.py` | 匹配小写字母开头的 `.py` 文件 |
| N18 | 字符类 `[A-Z]` | 项目根目录 | `**/[A-Z]*` | 匹配大写字母开头的条目 |
| N19 | 字符类 `[jt]` | 项目根目录 | `**/*.[jt]s` | 匹配 `.js` 或 `.ts` 文件 |
| N20 | 前缀通配符 | 项目根目录 | `**/*test*` | 匹配文件名中任意位置含 `test` 的条目 |
| N21 | 后缀通配符 | 项目根目录 | `**/*adme*` | 匹配文件名中任意位置含 `adme` 的条目 |
| N22 | `**/*` 全量匹配 | 项目根目录 | `**/*` | 返回所有文件和目录（含隐藏文件），递归展开 |
| N23 | `**` 仅目录匹配 | 项目根目录 | `**` | 仅返回所有目录（含根目录自身），不返回文件 |
| N24 | 目录尾 `/` 匹配 | 项目根目录 | `**/*/` | 仅返回所有子目录（以 `/` 结尾） |
| N25 | 匹配特定目录 | 项目根目录 | `**/config` | 匹配 `config/` 目录本身 |
| N26 | 大小写不敏感 | 项目根目录 | `**/*.PY` | 与 `**/*.py` 结果一致（大小写不敏感） |

### 7.4.3 边界/异常场景

| # | 用例名称 | path | pattern | 预期结果 |
|---|---------|------|---------|----------|
| E1 | path 为文件 | 某个文件的路径 | `*` | ⚠️ `Warning: [NOT_DIR] 目标是文件，不是目录` |
| E2 | path 不存在 | `/workspace/nonexistent` | `*` | ⚠️ `Warning: [NOT_FOUND] 路径不存在` |
| E3 | path 为根目录 `/` | `/` | `*` | ❌ `[PATH_IS_ROOT] 路径不能是根目录 '/'；请使用子目录如 '/workspace'` |
| E4 | path 含 `..` | `/workspace/demo_project` | `**/../**/*.py` | ❌ `[PATH_TRAVERSAL] path 不能包含 '..' 路径穿越` |
| E5 | 空 pattern | 项目根目录 | `""` | ⚠️ `Warning: [INVALID] 搜索模式不能为空` |
| E6 | pattern 无匹配 | 项目根目录 | `**/*.pyx` | `No matches found.` |
| E7 | pattern 指向不存在子目录 | 项目根目录 | `**/nonexistent/*` | `No matches found.` |
| E8 | 空目录搜索 | 空目录路径 | `*` | `No matches found.` |
| E9 | 花括号扩展（不支持） | 项目根目录 | `**/*.{py,json}` | `No matches found.` |
| E10 | pattern 指向文件的子路径 | 项目根目录 | `**/*.json/**` | `No matches found.`（文件无子项） |

### 7.4.4 显示格式验证

| # | 验证点 | 预期行为 |
|---|--------|----------|
| F1 | 文件输出格式 | `完整绝对路径(文件大小)`，如 `/workspace/demo_project/src/main.py(1 KB)` |
| F2 | 目录输出格式 | `完整绝对路径/`，目录名后有 `/`，无大小信息 |
| F3 | 大小单位 | 自适应：B / KB / MB / GB |
| F4 | 隐藏文件 | 点号 `.` 开头文件可被匹配和显示 |
| F5 | 特殊字符路径 | Emoji、Unicode、`!@#` 等特殊字符在路径中正常显示 |
| F6 | 排序规则 | 按完整路径 ASCII 字母序排列，文件与目录混合排序 |
| F7 | 空结果 | 统一返回 `No matches found.` |

### 7.4.5 `glob` 与 `grep` 对比
...
| **返回内容** | 匹配的路径列表 | 匹配的行内容 |
| **返回格式** | `路径(大小)` | `路径:行号: 内容` |
| **正则支持** | ❌ 仅 glob 通配符 | ✅ Python 正则 / 字面量 |
| **文件过滤** | ✅ 通过 pattern 直接实现 | ✅ 通过 `glob` 参数实现 |
| **path 为文件** | ⚠️ `[NOT_DIR]` 警告 | ✅ 支持搜索单文件 |
...
| 日期 | 版本 | 变更说明 |
|------|------|---------|
| 2026-01 | v2.1 | 新增第七章 `glob`（文件与目录匹配搜索）：含工具概述、参数规范、glob 模式说明、返回值规范、26 个正常用例 + 10 个边界/异常用例、显示格式验证、与 `grep` 对比 |
